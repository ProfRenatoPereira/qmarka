...
const htmlPromotionalProducts = document.querySelector("[data-products-news]")

function getTaggedProducts(products){
    let list = [];
    try{
        for (let i=0 ;i<products.conexaoConvertida.length;++i ){
            if (products.conexaoConvertida[i].tag01){
                list.push(products.conexaoConvertida[i]);
            }
        }
        if (list.length == 0){
            htmlPromotionalProducts.style.textAlign = "center"
            htmlPromotionalProducts.innerHTML = "Nenhum produto novo no momento! Volte mais tarde.";
        } else {
...
getTaggedProducts(await conectApi.productsList());
